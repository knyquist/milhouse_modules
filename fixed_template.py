...
from PlsH5IO import PlsH5Reader
...
def plotAlignmentDensityByTemplatePosition(cmph5, mh_name):
	...
	density_ends, bin_edges = np.histogram(tEnds, density=True, bins=range(0, 25000, 500))
	density_starts, bin_edges = np.histogram(tStarts, density=True, bins=range(0, 25000, 500))
	centers = []
...
def plot_templatePositionSurvival(cmph5, mh_name):
	...
	xlim_max = np.array(cg_x)[1:][np.max([index for index, val in enumerate(cg_y[1:]) if val > 0.1*np.max(cg_y[1:]) and index > ymax_index+1])]/1000.
	ax.set_xlim([0, xlim_max])
	rate = np.array(-dydx_over_S*100.)[np.where(np.array(cg_x)/1000. < xlim_max)]
...
def _getZMWRead(basecalls, zmw):
	...
	for reader in basecalls.parts:
		if np.min(reader.allSequencingZmws) < zmw and np.max(reader.allSequencingZmws) > zmw:
			baxreader = reader
			break  
...
def plot_tStart_vs_alnTimeStart(cmph5, bash5, plsh5, mh_name): 
	...
	fig = plt.figure(figsize=(14, 8), dpi=200)
	ax = fig.add_subplot(111)
	ax.scatter(np.array(alignment_start_time)/(frame_rate*60.), np.array(alignment_tStart)/1000., alpha=0.25, marker='.', color='blue', s=30)
...
def plot_tSpan_vs_tStart_or_tEnd(cmph5, mh_name): # cmph5s is a list of CmpH5Readers, names is list of milhouse Name fields
	...
	ax.set_title(mh_name, fontdict=font)
	ax.scatter(tStarts/1000., tSpans/1000., alpha=0.25, marker='.', color='red', s=15)
	...
	ax.scatter(tEnds/1000., tSpans/1000., alpha=0.25, marker='.', color='blue', s=15)
	ax.set_xlim([-0.5, np.max(tEnds)/1000.])
	ax.set_ylim([-0.5, np.max(tSpans)/1000.])
